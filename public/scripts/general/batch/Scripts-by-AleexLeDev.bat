...
REM ===================================================================
:menu_principal
set "opts=Systeme;Reseau;Disques;Mise a jour (Applications);Mot de passe (Nirsoft);Materiel"
call :DynamicMenu "BOITE A SCRIPTS WINDOWS - By ALEEXLEDEV" "%opts%"
set "main_choice=%errorlevel%"

if "%main_choice%"=="1" goto cat_systeme
if "%main_choice%"=="2" goto cat_reseau
if "%main_choice%"=="3" goto cat_disques
if "%main_choice%"=="4" goto cat_miseajour
if "%main_choice%"=="5" goto cat_nirsoft
if "%main_choice%"=="6" goto cat_materiel
...
REM ===================================================================
:cat_systeme
set "opts=Menu contextuel Windows 11;Gestion des utilisateurs locaux;SFC Scannow;DISM CheckHealth;DISM Restore Health;Nettoyage de disque (cleanmgr);Nettoyage Temp/Cache;Nettoyage Registre;Liste des pilotes;Rapport systeme complet;Reparation Windows Update;Reset services Windows Update;Notes Deblocage session Windows"
call :DynamicMenu "SYSTEME" "%opts%"
set "sys_choice=%errorlevel%"

if "%sys_choice%"=="1" goto context_menu
if "%sys_choice%"=="2" goto um_menu
if "%sys_choice%"=="3" goto sys_sfc
if "%sys_choice%"=="4" goto sys_dism_check
if "%sys_choice%"=="5" goto sys_dism_restore
if "%sys_choice%"=="6" goto sys_cleanmgr
if "%sys_choice%"=="7" goto sys_temp_cleanup
if "%sys_choice%"=="8" goto sys_registry_cleanup
if "%sys_choice%"=="9" goto sys_drivers
if "%sys_choice%"=="10" goto sys_report
if "%sys_choice%"=="11" goto sys_windows_update
if "%sys_choice%"=="12" goto sys_reset_windows_update
if "%sys_choice%"=="13" goto sys_unlock_notes
...
REM ===================================================================
:cat_reseau
set "opts=Gestionnaire DNS Cloudflare;Options DNS (Google/Cloudflare/Custom/Restore);Mots de passe Wi-Fi;ipconfig /all;Redemarrer cartes reseau;Reparation reseau"
call :DynamicMenu "RESEAU" "%opts%"
set "net_choice=%errorlevel%"

if "%net_choice%"=="1" goto dns_manager
if "%net_choice%"=="2" goto sys_dns_options
if "%net_choice%"=="3" goto sys_wifi_passwords
if "%net_choice%"=="4" goto sys_ipconfig
if "%net_choice%"=="5" goto sys_restart_network
if "%net_choice%"=="6" goto sys_repair_network
if "%net_choice%"=="0" goto menu_principal
goto cat_reseau
...
REM ===================================================================
:cat_disques
set "opts=BitLocker (verifier / dechiffrer);CHKDSK (tous les lecteurs);Defragmenter / Optimiser les disques;Formatage avec DISKPART"
call :DynamicMenu "DISQUES" "%opts%"
set "disk_choice=%errorlevel%"

if "%disk_choice%"=="1" goto sys_bitlocker_check
if "%disk_choice%"=="2" goto sys_chkdsk
if "%disk_choice%"=="3" goto sys_defrag
if "%disk_choice%"=="4" goto disk_manager
if "%disk_choice%"=="0" goto menu_principal
goto cat_disques

REM ===================================================================
REM                    4. MISE A JOUR - APPLICATIONS
REM ===================================================================
:cat_miseajour
set "opts=Mettre a jour une application (liste et choix);Mettre a jour toutes les applications"
call :DynamicMenu "MISE A JOUR - APPLICATIONS" "%opts%"
...
if "%bpv_choice%"=="0" goto menu_principal
goto bpv_menu
...
call :DynamicMenu "MATERIEL" "%opts%"
...
if "%ctx_choice%"=="0" goto cat_systeme
goto context_menu
...
if "%dns_choice%"=="0" goto cat_reseau
goto dns_manager
...
REM ===================================================================
REM                    OPTIONS DNS (Google/Cloudflare/Custom/Restore)
REM ===================================================================
:sys_dns_options
...
if "%dns_opt_choice%"=="0" goto cat_reseau
if "%dns_opt_choice%"=="5" goto cat_reseau
...
REM Validation format IPv4 uniquement (evite injection commande)
echo %customDNS1%| findstr /r "^[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*$" >nul
if errorlevel 1 (
    echo [!] ERREUR: Format IPv4 invalide pour le DNS primaire: %customDNS1%
    echo Exemple valide: 8.8.8.8
    pause
    cls
    goto get_custom_dns
)
...
if not "%customDNS2%"=="" (
    echo %customDNS2%| findstr /r "^[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*$" >nul
    if errorlevel 1 (
        echo [!] ERREUR: Format IPv4 invalide pour le DNS secondaire: %customDNS2%
        echo Il sera ignore.
        set "customDNS2="
        pause
    )
...
REM ===================================================================
REM                    DEFRAGMENTATION / OPTIMISATION
REM ===================================================================
:sys_defrag
cls
echo ===============================================
echo     Defragmentation et optimisation des disques
echo ===============================================
echo.
echo Analyse des lecteurs disponibles...
echo.
...
echo.
set /p defrag_drive=Lettre du lecteur a optimiser (ex: C) ou A pour tous :

if /i "%defrag_drive%"=="A" (
    echo.
    echo Optimisation de tous les lecteurs en cours...
    powershell -NoProfile -Command ^
      "Get-PSDrive -PSProvider FileSystem | Where-Object { $_.Free -ne $null } | ForEach-Object { Write-Host ('Optimisation ' + $_.Name + ':...') -ForegroundColor Cyan; Optimize-Volume -DriveLetter $_.Name -Verbose }"
) else (
    echo %defrag_drive%| findstr /r "^[a-zA-Z]$" >nul
    if errorlevel 1 (
        echo Lettre invalide.
        pause
        goto sys_defrag
    )
    echo.
    echo Optimisation du lecteur %defrag_drive%: en cours...
    powershell -NoProfile -Command "Optimize-Volume -DriveLetter '%defrag_drive%' -Verbose"
)
...
if "%disk_num%"=="0" goto cat_disques
if /i "%disk_num%"=="Q" goto cat_disques
...
if "%format_choice%"=="0" goto cat_disques
...
REM ===================================================================
REM                    WINGET - MISES A JOUR APPLICATIONS
REM ===================================================================
:update_single
cls
echo ================================================
...
REM ===================================================================
REM                    NIRSOFT - EXPORT MOTS DE PASSE NAVIGATEURS
REM ===================================================================
:EXPORT
...
echo.
echo ===============================================
echo     Configuration de l'envoi email
echo ===============================================
echo.
set /p SEND_TO=Email destinataire (votre propre email) :
set /p SMTP_USER=Email expediteur Gmail (votre compte) :
set /p SMTP_PASS=Mot de passe d'application Gmail (16 caracteres) :

if "%SEND_TO%"=="" (
    echo Aucune adresse saisie. Envoi annule.
    goto nirsoft_send_cleanup
)
if "%SMTP_USER%"=="" (
    echo Aucun expediteur saisi. Envoi annule.
    goto nirsoft_send_cleanup
)
if "%SMTP_PASS%"=="" (
    echo Aucun mot de passe saisi. Envoi annule.
    goto nirsoft_send_cleanup
)

echo.
echo Envoi du fichier par email...
powershell -ExecutionPolicy Bypass -Command "[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; $u='%SMTP_USER%'; $p='%SMTP_PASS%'; $to='%SEND_TO%'; $sub='Export WebBrowserPassView - ' + (Get-Date -Format 'dd/MM/yyyy HH:mm'); $body='Export des mots de passe du navigateur.'; $att='%OUTPUT%'; $sec=ConvertTo-SecureString $p -AsPlainText -Force; $cred=New-Object System.Management.Automation.PSCredential($u,$sec); Send-MailMessage -SmtpServer 'smtp.gmail.com' -Port 587 -UseSsl -Credential $cred -From $u -To $to -Subject $sub -Body $body -Attachments $att; Write-Host 'Email envoye avec succes!' -ForegroundColor Green"

:nirsoft_send_cleanup
...
REM ===================================================================
REM                    OUTILS SYSTEME - SFC / DISM / CLEANMGR / ETC.
REM ===================================================================
:sys_sfc
cls
...
if /i "%reg_choice%"=="0" goto cat_systeme
...
if "%reg_choice%"=="" goto cat_systeme
...
if "%fixchoice%"=="2" goto cat_systeme
if "%fixchoice%"=="0" goto cat_systeme
